...
namespace Latios.KAG50
{
    ...
    {
        public static void AddAnimationNodeBufferInternal(IBaker baker, Entity entity)
        {
            //node data
            baker.AddBuffer<FinalPoseNodeState>(   entity);
            baker.AddBuffer<SingleClipNodeState>(  entity);
            baker.AddBuffer<StateMachineNodeState>(entity);
        }
        ...
        private static void RecursionAnimationNode(ref PoseLinkBlob poseLink,
                                                   ...
                                                   ref DynamicBuffer<StateMachineNodeState> stateMachineNodeStates)
        {
            var matcher = new StateMatcher { stateIndex = stateIndex };
            switch (poseLink.LinkNodeType)
            {
                case AnimationNodeType.SingleClip:
                {
                    sbyte               index          = singleClipNodeStates.GetIndexByBufferId(ref poseLink.LinkId, matcher);
                ...
                {
                    sbyte                 index            = stateMachineNodeStates.GetIndexByBufferId(ref poseLink.LinkId, matcher);
        ...
        struct StateMatcher : IMatcher<SingleClipNodeState>, IMatcher<StateMachineNodeState>
        {
            public sbyte stateIndex;

            public bool Matches(in SingleClipNodeState candidate) => candidate.m_StateIndex == stateIndex;
            public bool Matches(in StateMachineNodeState candidate) => candidate.m_StateIndex == stateIndex;
        }
    }
