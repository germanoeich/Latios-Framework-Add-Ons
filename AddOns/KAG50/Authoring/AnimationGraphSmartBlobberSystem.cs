...
{
    public static class AnimationGraphSmartBlobberExtensions
    {
        public static SmartBlobberHandle<AnimationGraphBlob> RequestCreateBlobAsset(this IBaker baker, AnimationGraphAsset graphAsset)
        {
            var filter = new AnimationGraphSmartBlobberFilter { AnimationGraphAsset = graphAsset };
            return baker.RequestCreateBlobAsset<AnimationGraphBlob, AnimationGraphSmartBlobberFilter>(filter);
        }
    }

    public struct AnimationGraphSmartBlobberFilter : ISmartBlobberRequestFilter<AnimationGraphBlob>
    {
        ...
        public bool Filter(IBaker baker, Entity blobBakingEntity)
        {
            ...
            baker.DependsOn(AnimationGraphAsset);
            baker.AddComponent(blobBakingEntity, new AnimationGraphAssetRef { graphAsset = AnimationGraphAsset });
            return true;
        }
    }

    internal struct AnimationGraphAssetRef : IComponentData
    {
        public UnityObjectRef<AnimationGraphAsset> graphAsset;
    }
...
namespace Latios.KAG50.Asset.Systems
{
    [WorldSystemFilter(WorldSystemFilterFlags.BakingSystem)]
    [UpdateInGroup(typeof(SmartBlobberBakingGroup))]
    internal partial class AnimationGraphSmartBlobberSystem : SystemBase
    {
        protected override void OnCreate()
        {
            base.OnCreate();
            new SmartBlobberTools<AnimationGraphBlob>().Register(World);
        }

        protected override void OnUpdate()
        {
            foreach ((var result, var graph) in SystemAPI.Query<RefRW<SmartBlobberResult>,
                                                                AnimationGraphAssetRef>().WithOptions(EntityQueryOptions.IncludePrefab |
                                                                                                      EntityQueryOptions.IncludeDisabledEntities))
            {
                var converter       = CreateConverter(graph.graphAsset, Allocator.Temp);
                result.ValueRW.blob = UnsafeUntypedBlobAssetReference.Create(converter.BuildBlob());
            }
        }
